#!/usr/bin/env python3
"""acceptance-verifier-v3.8.py

Controller-owned strict acceptance verifier for the v3.8 FAIL-CLOSED package.

Phases (--phase):
  functional-continuity  : v3.8 artifacts deep-equal v3.4/v3.5 functional contract
...
V38_BATCH = "BATCH-20260808T132428Z-DOGFOOD004-LAB-V3_8"
V37_MANIFEST = "package-payload-manifest-v3.8.json"
...
class Verifier:
    ...
    def phase_functional_continuity(self, pkg_dir):
        api = load_json(os.path.join(pkg_dir, "lab-api-contract-v3.8.json"))
        baseline = load_json(os.path.join(pkg_dir, "functional-contract-baseline-v3.8.json"))
        batch = load_json(os.path.join(pkg_dir, "batch-manifest-v3.8.json"))
        self.require(api["modules"] == baseline["canonical_contract_modules"], "modules deep-equal v3.4/v3.5")
        self.require(api["version"] == "v3.8", "api version v3.8")
        ...
        self.require(baseline["supersedes"] == "v3.7", "baseline supersedes v3.7")
        self.require(batch["supersedes"]["package"] == "v3.7", "supersedes v3.7 recorded")
    ...
    def phase_r_only(self, repo, C, R, owned, evidence_path, batch_manifest):
        ...
        wt = tempfile.mkdtemp(prefix="verifier-v38-r-")
        try:
    ...
    def phase_identity_coherence(self, pkg_dir, lab=None, evidence_path=None,
                                  acceptance=None, repo=None, C=None, R=None, E=None,
                                  planned_branch=None, remote=None):
        """ID-F4 (v3.8): independent cross-manifest identity graph. Every
        identity source must agree with the authoritative package identity
        (package-identity-v3.8.json). PASS only if identity_sources_checked
        == N and mismatches == []. full-completion requires this phase PASS."""
        auth = load_json(os.path.join(pkg_dir, "package-identity-v3.8.json"))
        sources = {
            "authority": auth,
            "scope": load_json(os.path.join(pkg_dir, "scope-freeze-v3.8.json")),
            "batch-manifest": load_json(os.path.join(pkg_dir, "batch-manifest-v3.8.json")),
            "dispatch": load_json(os.path.join(pkg_dir, "dispatch-plan-v3.8.json")),
            "lease-A": load_json(os.path.join(pkg_dir, "lease-plan-lab-a-v3.8.json")),
            "lease-B": load_json(os.path.join(pkg_dir, "lease-plan-lab-b-v3.8.json")),
            "predispatch": load_json(os.path.join(pkg_dir, "predispatch-readiness-v3.8.json")),
        }
        self.require(len(sources) == 7, "7 identity sources loaded")
        n = 0
        # batch_id equality across ALL sources
        for name, src in sources.items():
            n += 1
            self.require(src.get("batch_id") == auth.get("batch_id"),
                         f"{name} batch == authority batch",
                         f"{src.get('batch_id')} vs {auth.get('batch_id')}")
        # package equality for sources that declare it
        for name, src in sources.items():
            if "package" in src:
                n += 1
                self.require(src.get("package") == auth.get("package"),
                             f"{name} package == authority package",
        ...
        for name, src in sources.items():
            if src.get("candidate_head_sha") is not None:
                n += 1
                self.require(src.get("candidate_head_sha") == auth.get("candidate_head_sha"),
                             f"{name} candidate == authority candidate",
                             f"{src.get('candidate_head_sha')} vs {auth.get('candidate_head_sha')}")
        # supersedes identity: batch-manifest == authority
        bm = sources["batch-manifest"]
        for field in ("package", "batch_id", "manifest_sha256"):
            n += 1
            self.require(bm.get("supersedes", {}).get(field) == auth.get("supersedes", {}).get(field),
                         f"supersedes {field} == authority",
                         f"{bm.get('supersedes', {}).get(field)} vs {auth.get('supersedes', {}).get(field)}")
        # per-lab leaf/lease/result-path/branch-pattern mapping
        for lkey in ("LAB-A", "LAB-B"):
            acfg = auth.get("labs", {}).get(lkey)
            lpl = sources["lease-" + lkey[-1]]
            n += 4
            self.require(acfg is not None, f"{lkey} registered in authority")
            self.require(lpl.get("leaf_id") == acfg.get("leaf_id"), f"{lkey} leaf id == authority",
                         f"{lpl.get('leaf_id')} vs {acfg.get('leaf_id')}")
            self.require(lpl.get("lease_id") == acfg.get("lease_id"), f"{lkey} lease id == authority",
                         f"{lpl.get('lease_id')} vs {acfg.get('lease_id')}")
            self.require(lpl.get("result_path") == acfg.get("result_evidence_path"),
                         f"{lkey} result path == authority",
                         f"{lpl.get('result_path')} vs {acfg.get('result_evidence_path')}")
            self.require(branch_matches_authority_pattern(
                lpl.get("planned_result_branch"), lkey, acfg.get("planned_result_branch")),
                f"{lkey} branch pattern == authority",
                f"{lpl.get('planned_result_branch')}")
        # evidence cross-check against the identity graph (independent of evidence.batch)
        if evidence_path and repo and E and lab:
            ev = None
            try:
                blob = subprocess.run(["git", "-C", repo, "cat-file", "blob", f"{E}:{evidence_path}"],
                                      capture_output=True).stdout
                ...
                ev = None
            if ev is not None:
                acfg = auth.get("labs", {}).get(lab)
                n += 5
                self.require(ev.get("batch") == auth.get("batch_id"), "evidence batch == authority batch")
                self.require(ev.get("C") == auth.get("candidate_head_sha"), "evidence C == authority candidate")
                self.require(ev.get("lab") == lab, "evidence lab exact")
                self.require(ev.get("evidence_path") == acfg.get("result_evidence_path"),
                             "evidence path == authority")
                self.require(branch_matches_authority_pattern(ev.get("planned_branch"), lab,
                                                              acfg.get("planned_result_branch")),
                             "evidence planned branch == authority pattern")
        # acceptance cross-check against the identity graph
        if acceptance and os.path.exists(acceptance):
            a = load_json(acceptance)
            acfg = auth.get("labs", {}).get(a.get("lab")) or {}
            n += 3
            self.require(a.get("batch") == auth.get("batch_id"), "A batch == authority batch")
            self.require(a.get("C") == auth.get("candidate_head_sha"), "A C == authority candidate")
            self.require(a.get("evidence_path") == acfg.get("result_evidence_path"),
                         "A evidence path == authority")
        self.phase_results.setdefault("identity_coherence", {})["identity_sources_checked"] = n
        self.phase_results.setdefault("identity_coherence", {})["mismatches"] = []
        return not self.errors

    def phase_full(self, pkg_dir, repo, C, R, E, evidence_path, acceptance, owned, batch_manifest,
        ...
        print("== full-completion: identity coherence (ID-F4) ==")
        self.phase_identity_coherence(pkg_dir, lab=lab, evidence_path=evidence_path,
                                      acceptance=acceptance, repo=repo, C=C, R=R, E=E,
                                      planned_branch=planned_branch, remote=remote)
        ...
        wt = tempfile.mkdtemp(prefix="verifier-v38-full-")
        try:
...
def build_selftest():
    ...
    results = {}
    base = tempfile.mkdtemp(prefix="v38-selftest-")
    print(f"selftest base: {base}")

    PKG = os.path.dirname(os.path.abspath(__file__))  # package dir
    FINALIZER = os.path.join(PKG, "mechanical-finalizer-v3.8.py")
    VERIFIER = os.path.join(PKG, "acceptance-verifier-v3.8.py")
    SF = os.path.join(PKG, "scope-freeze-v3.8.json")
    BM = os.path.join(PKG, "batch-manifest-v3.8.json")
    FB = os.path.join(PKG, "functional-contract-baseline-v3.8.json")
    API = os.path.join(PKG, "lab-api-contract-v3.8.json")
    BP = os.path.join(PKG, "build-provenance-policy-v3.8.json")
    IA = os.path.join(PKG, "package-identity-v3.8.json")
    LEASE_A = os.path.join(PKG, "lease-plan-lab-a-v3.8.json")
    LEASE_B = os.path.join(PKG, "lease-plan-lab-b-v3.8.json")

    # v3.8 identity fixtures: C MUST be the real frozen candidate
    # (509538784afc...). The finalizer identity gate (ID-F3) rejects any
    # fixture whose --c differs from the authoritative candidate, so fixtures
    # are sparse worktrees of a --shared clone of the real repo at C, and R is
    # committed on top of C (owned synthetic modules only).
    CANDIDATE = "509538784afc2b828f2d922f65cf8ca3a39b5ee7"
    REAL_REPO = os.environ.get("SEQ66_REAL_REPO",
                               "/home/ubuntu/code/music/seq66-loves-sooperlooper")
    ...
    sh(["git", "-C", SEED, "config", "user.email", "v38@selftest"])
    sh(["git", "-C", SEED, "config", "user.name", "selftest"])
    sh(["git", "-C", SEED, "checkout", "-b", "C", CANDIDATE], timeout=300)

    def fresh_repo(name):
        d = os.path.join(base, name)
        # FULL (non-sparse) worktree: evidence/build fixtures live outside the
        # tests/ sparse cone and git refuses to add paths outside the cone.
        sh(["git", "-C", SEED, "-c", "core.sparseCheckout=false",
            "worktree", "add", "--detach", d, CANDIDATE], timeout=300)
        return d

    def commit_all(repo, msg):
        if msg == "C":
            # C is the real frozen candidate; the synthetic owned modules are
            # committed together with the R marker (single R commit on top of C).
            return CANDIDATE
        sh(["git", "-C", repo, "add", "-A"])
        r = sh(["git", "-C", repo, "commit", "-q", "-m", msg], timeout=120)
        return git(repo, "rev-parse", "HEAD")

    # ---- realistic module fixtures implementing the v3.8 contract surface ----
...
""")
    ...
    def write_lab_b_modules(repo):
        ...
        with open(os.path.join(repo, "tests/integration/__init__.py"), "w") as f:
            f.write("# integration\n")
        # v3.8 real-C fixtures: headless_audio_lab/__init__.py is LAB-A-owned and
        # does NOT exist in the real candidate. Python 3 namespace packages make
        # it optional for LAB-B imports; creating it would put a non-owned path
        # in LAB-B's C..R diff, so it is intentionally NOT created here.
        with open(os.path.join(repo, owned[2]), "w") as f:
...
""")
        ...
        with open(os.path.join(repo, owned[11]), "w") as f:
            f.write("*\n!.gitkeep\n")
        # v3.8 real-C fixtures: synthetic_source/ and deterministic_capture/ do not
        # exist in the real candidate; the .gitkeep files materialize the owned
        # DIRECTORIES (required-paths gate) and are owned via dir-prefix
        # ownership (topology gate).
        ...
        # fixtures/.gitignore ('*' rule) would ignore itself and the .gitkeeps;
        # force-add so the owned fixture dirs stay tracked.
        sh(["git", "-C", repo, "add", "-f",
...
""")
    ...
    rv = sh(["python3", VERIFIER, "--package-dir", PKG, "--phase", "r-only-leaf-result", "--repo", repo,
             "--c", C, "--r", R1, "--lab", "LAB-A",
             "--evidence-path", "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json"],
            timeout=300)
    ...
    evp = "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json"
    fz = sh(["python3", FINALIZER, "--mode", "finalize", "--repo", repo, "--c", C, "--r", R2,
             "--lab", "LAB-A", "--batch", V38_BATCH, "--planned-branch", "result/x-lab-a-v3_8",
             "--remote", "", "--scope-freeze", SF, "--batch-manifest", BM,
             "--functional-baseline", FB, "--api-contract", API, "--build-policy", BP,
             "--controller-dir", repo, "--evidence-path", evp, "--verifier-path", VERIFIER, "--identity-authority", IA, "--lease-plan", LEASE_A],
            timeout=600)
    ...
    base_args = ["python3", FINALIZER, "--mode", "reproduce", "--repo", repo3, "--c", C3, "--r", R3,
                 "--lab", "LAB-A", "--batch", V38_BATCH, "--planned-branch", "result/x-lab-a-v3_8",
                 "--remote", "", "--scope-freeze", SF, "--batch-manifest", BM,
                 "--identity-authority", IA, "--lease-plan", LEASE_A,
                 "--functional-baseline", FB, "--api-contract", API, "--build-policy", BP,
                 "--controller-dir", repo3, "--evidence-path", "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json",
    ...
    evp4 = "receipts/headless-lab-results-v3.8/lab-b-controller-evidence.json"
    fzb = sh(["python3", FINALIZER, "--mode", "finalize", "--repo", repo4, "--c", C4, "--r", R4,
              "--lab", "LAB-B", "--batch", V38_BATCH, "--planned-branch", "result/x-lab-b-v3_8",
              "--remote", "", "--scope-freeze", SF, "--batch-manifest", BM,
              "--functional-baseline", FB, "--api-contract", API, "--build-policy", BP,
              "--controller-dir", repo4, "--evidence-path", evp4, "--verifier-path", VERIFIER, "--identity-authority", IA, "--lease-plan", LEASE_B],
             timeout=600)
    ...
    sh(["git", "-C", repo5, "remote", "set-url", "origin", bare5])
    ls = sh(["git", "-C", repo5, "ls-remote", "--exit-code", bare5, "refs/heads/result/x-lab-a-v3_8"], timeout=120)
    ...
    sh(["git", "-C", repo6, "remote", "set-url", "origin", bare6])
    push = sh(["git", "-C", repo6, "push", "-q", "origin", f"{R6}:refs/heads/result/ctl-lab-a-v3_8"], timeout=120)
    ls6 = sh(["git", "-C", repo6, "ls-remote", "origin", "refs/heads/result/ctl-lab-a-v3_8"]).stdout.strip()
    ...
    def run_finalizer(repo, C, R, lab, branch, extra=None, evp=None):
        args = ["python3", FINALIZER, "--mode", "finalize", "--repo", repo, "--c", C, "--r", R,
                "--lab", lab, "--batch", V38_BATCH, "--planned-branch", branch,
                "--remote", "", "--scope-freeze", SF, "--batch-manifest", BM,
                "--functional-baseline", FB, "--api-contract", API, "--build-policy", BP,
                "--controller-dir", repo,
                "--evidence-path", evp or f"receipts/headless-lab-results-v3.8/{lab.lower()}-controller-evidence.json",
                "--identity-authority", IA, "--lease-plan", (LEASE_A if lab == "LAB-A" else LEASE_B),
    ...
    args = ["python3", FINALIZER, "--mode", "finalize", "--repo", repo, "--c", C, "--r", R,
            ...
            "--identity-authority", IA, "--lease-plan", LEASE_A,
            "--functional-baseline", FB, "--api-contract", API, "--build-policy", BP,
            "--controller-dir", repo, "--evidence-path", "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json",
            "--verifier-path", VERIFIER]
    rr = sh(args, timeout=600)
    evp_after = os.path.join(repo, "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json")
    ...
    OWNED_A = write_lab_a_modules(repo, "# module v1")
    C = commit_all(repo, "C")
    evp2 = "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json"
    os.makedirs(os.path.join(repo, os.path.dirname(evp2)), exist_ok=True)
    with open(os.path.join(repo, evp2), "w") as f:
        f.write("{}")
    with open(os.path.join(repo, OWNED_A[0]), "a") as f:
        f.write("# R\n")
    # force-add: the real repo .gitignore ignores *.json; the evidence MUST be
    # part of R so check_evidence_absent rejects it (fail-closed, not silent).
    sh(["git", "-C", repo, "add", "-f", evp2])
    R = commit_all(repo, "R")
    rr = run_finalizer(repo, C, R, "LAB-A", "result/x-lab-a-v3_8", evp=evp2)
    ...
    with open(os.path.join(repo, "build/out.bin"), "w") as f:
        f.write("junk")
    # force-add: the real repo .gitignore ignores build/; the junk output MUST
    # land in R so the ownership/topology gate rejects it.
    sh(["git", "-C", repo, "add", "-f", "build/out.bin"])
    R = commit_all(repo, "R")
    rr = run_finalizer(repo, C, R, "LAB-A", "result/x-lab-a-v3_8")
    ...
    with open(os.path.join(repo, OWNED_B[2]), "a") as f:
        f.write("# R\n")
    R = commit_all(repo, "R")
    rr = run_finalizer(repo, C, R, "LAB-B", "result/x-lab-b-v3_8")
    ...
    with open(os.path.join(repo, OWNED_A[0]), "a") as f:
        f.write("# R\n")
    R = commit_all(repo, "R")
    evp8 = "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json"
    fz8 = sh(["python3", FINALIZER, "--mode", "finalize", "--repo", repo, "--c", C, "--r", R,
              "--lab", "LAB-A", "--batch", V38_BATCH, "--planned-branch", "result/x-lab-a-v3_8",
              "--remote", "", "--scope-freeze", SF, "--batch-manifest", BM,
              "--functional-baseline", FB, "--api-contract", API, "--build-policy", BP,
              "--controller-dir", repo, "--evidence-path", evp8, "--verifier-path", VERIFIER, "--identity-authority", IA, "--lease-plan", LEASE_A],
             timeout=600)
    E8 = json.loads(fz8.stdout).get("E") if fz8.returncode == 0 else None
    bare8 = os.path.join(base, "bare-n8")
    os.makedirs(bare8)
    sh(["git", "-C", bare8, "init", "-q", "--bare"])
    sh(["git", "-C", repo, "remote", "set-url", "origin", bare8])
    # pre-create the branch with a different commit
    sh(["git", "-C", repo, "push", "-q", "origin", f"{C}:refs/heads/result/x-lab-a-v3_8"], timeout=120)
    if E8:
        rv8 = sh(["python3", VERIFIER, "--package-dir", PKG, "--phase", "prepublish-finalization", "--repo", repo,
                  "--c", C, "--r", R, "--e", E8, "--lab", "LAB-A",
                  "--evidence-path", evp8, "--planned-branch", "result/x-lab-a-v3_8",
                  "--remote", bare8, "--finalizer", "python3",
                  "--finalizer-args", f"{FINALIZER} --repo {repo} --c {C} --r {R} --lab LAB-A --batch {V38_BATCH} --planned-branch result/x-lab-a-v3_8 --scope-freeze {SF} --batch-manifest {BM} --identity-authority {IA} --lease-plan {LEASE_A} --functional-baseline {FB} --api-contract {API} --build-policy {BP} --controller-dir {repo} --evidence-path {evp8} --verifier-path {VERIFIER}",
                  "--verifier-path", VERIFIER, "--identity-authority", IA, "--lease-plan", LEASE_A], timeout=600)
    ...
    with open(os.path.join(repo, OWNED_A[0]), "a") as f:
        f.write("# R\n")
    R = commit_all(repo, "R")
    evp9 = "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json"
    fz9 = sh(["python3", FINALIZER, "--mode", "finalize", "--repo", repo, "--c", C, "--r", R,
              "--lab", "LAB-A", "--batch", V38_BATCH, "--planned-branch", "result/ctl-lab-a-v3_8",
              "--remote", "", "--scope-freeze", SF, "--batch-manifest", BM,
              "--functional-baseline", FB, "--api-contract", API, "--build-policy", BP,
              "--controller-dir", repo, "--evidence-path", evp9, "--verifier-path", VERIFIER, "--identity-authority", IA, "--lease-plan", LEASE_A],
             timeout=600)
    E9 = json.loads(fz9.stdout).get("E") if fz9.returncode == 0 else None
    bare9 = os.path.join(base, "bare-n9")
    os.makedirs(bare9)
    sh(["git", "-C", bare9, "init", "-q", "--bare"])
    sh(["git", "-C", repo, "remote", "set-url", "origin", bare9])
    sh(["git", "-C", repo, "push", "-q", "origin", f"{E9}:refs/heads/result/ctl-lab-a-v3_8"], timeout=120) if E9 else None
    a9 = os.path.join(base, "acceptance-n9.json")
    json.dump({"schema": "controller-acceptance/v3.8", "lab": "LAB-A", "batch": V38_BATCH,
               "C": C, "R": "1111111111111111111111111111111111111111", "E": E9, "branch": "result/ctl-lab-a-v3_8",
    ...
    if E9:
        rv9 = sh(["python3", VERIFIER, "--package-dir", PKG, "--phase", "postpublish-binding", "--repo", repo,
                  ...
                  "--evidence-path", evp9, "--planned-branch", "result/ctl-lab-a-v3_8",
                  "--remote", bare9, "--acceptance", a9, "--finalizer", "python3",
                  "--finalizer-args", f"{FINALIZER} --repo {repo} --c {C} --r {R} --lab LAB-A --batch {V38_BATCH} --planned-branch result/ctl-lab-a-v3_8 --scope-freeze {SF} --batch-manifest {BM} --identity-authority {IA} --lease-plan {LEASE_A} --functional-baseline {FB} --api-contract {API} --build-policy {BP} --controller-dir {repo} --evidence-path {evp9} --verifier-path {VERIFIER}",
                  "--verifier-path", VERIFIER, "--identity-authority", IA, "--lease-plan", LEASE_A], timeout=600)
    ...
    with open(os.path.join(repo, OWNED_A[0]), "a") as f:
        f.write("# R\n")
    R = commit_all(repo, "R")
    evp10 = "receipts/headless-lab-results-v3.8/lab-a-controller-evidence.json"
    fz10 = sh(["python3", FINALIZER, "--mode", "finalize", "--repo", repo, "--c", C, "--r", R,
               "--lab", "LAB-A", "--batch", V38_BATCH, "--planned-branch", "result/x-lab-a-v3_8",
               "--remote", "", "--scope-freeze", SF, "--batch-manifest", BM,
               "--functional-baseline", FB, "--api-contract", API, "--build-policy", BP,
               "--controller-dir", repo, "--evidence-path", evp10, "--verifier-path", VERIFIER, "--identity-authority", IA, "--lease-plan", LEASE_A],
              timeout=600)
    ...
    if E10:
        ...
        rv10 = sh(["python3", VERIFIER, "--package-dir", PKG, "--phase", "full-completion", "--repo", repo,
                   ...
                   "--evidence-path", evp10, "--planned-branch", "result/x-lab-a-v3_8",
                   "--remote", "file:///nonexistent", "--acceptance", os.path.join(base, "no-a.json"),
                   "--finalizer", "python3",
                   "--finalizer-args", f"{FINALIZER} --repo {repo} --c {C} --r {R} --lab LAB-A --batch {V38_BATCH} --planned-branch result/x-lab-a-v3_8 --scope-freeze {SF} --batch-manifest {BM} --identity-authority {IA} --lease-plan {LEASE_A} --functional-baseline {FB} --api-contract {API} --build-policy {BP} --controller-dir {repo} --evidence-path {evp10} --verifier-path {VERIFIER}",
                   "--verifier-path", VERIFIER, "--identity-authority", IA, "--lease-plan", LEASE_A], timeout=600)
    ...
    # ---------------- FC-F1 runtime-contract negative fixtures (v3.8) ----------------
    def lab_b_finalize(repo, C, R):
        return run_finalizer(repo, C, R, "LAB-B", "result/x-lab-b-v3_8")
    ...
    def mutate_and_finalize(mutator, lab="LAB-B", lab_b=True):
        ...
        with open(os.path.join(repo, "tests/integration/headless_audio_lab/audio_oracle.py" if lab_b else "tests/integration/headless_audio_lab/process_supervisor.py"), "a") as f:
            f.write("# R\n")
        R = commit_all(repo, "R")
        rr = lab_b_finalize(repo, C, R) if lab_b else run_finalizer(repo, C, R, "LAB-A", "result/x-lab-a-v3_8")
        return rr
    ...
    # cleanup fixture worktrees + seed (disk: 6.9GB free on the runner)
    for sub in list(os.listdir(base)):
        p = os.path.join(base, sub)
        if sub == "seed" or not os.path.exists(os.path.join(p, ".git")):
            continue
        sh(["git", "-C", SEED, "worktree", "remove", "--force", p], timeout=120)
    shutil.rmtree(base, ignore_errors=True)
    return 0 if ok else 1

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--phase", choices=["identity-coherence", "functional-continuity", "r-only-leaf-result",
    ...
    ap.add_argument("--identity-authority", default=None)
    ap.add_argument("--lease-plan", default=None)
    args = ap.parse_args()
    ...
    v = Verifier(args)
    if args.phase == "identity-coherence":
        print("== identity-coherence ==")
        ok = v.phase_identity_coherence(pkg, lab=args.lab, evidence_path=args.evidence_path,
                                        acceptance=args.acceptance, repo=args.repo,
                                        C=args.c, R=args.r, E=args.e,
                                        planned_branch=args.planned_branch, remote=args.remote)
        return 0 if ok else 1
    ...
    if not (args.repo and args.c and args.r):
        ...
        return 2
    baseline = load_json(os.path.join(pkg, "functional-contract-baseline-v3.8.json"))
    owned = baseline["canonical_ownership"].get(args.lab, [])
    batch = load_json(os.path.join(pkg, "batch-manifest-v3.8.json"))
    evp = args.evidence_path or f"receipts/headless-lab-results-v3.8/{args.lab.lower()}-controller-evidence.json"
    ...
    if args.phase in ("full-completion", "all"):
        ...
        ok = v.phase_full(pkg, args.repo, args.c, args.r, args.e, evp, args.acceptance, owned, batch,
