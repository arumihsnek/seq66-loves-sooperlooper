#!/usr/bin/env python3
"""acceptance-verifier-v3.10.py — independent v3.10 acceptance gate verifier.
...
Verifies, independently of the finalizer, that:
  - the gate registry / traceability map is coherent (no orphan gates,
    ...
    are present in leaf-requirements-v3.10.json)
...
requirement omitted from leaf-requirements-v3.10.json.
...
class Verifier:
    ...
    def verify_traceability(self, pkg_dir, finalizer_output=None):
        ...
        trace = load_json(os.path.join(pkg_dir, "gate-traceability-v3.10.json"))
        leaf_req = load_json(os.path.join(pkg_dir, "leaf-requirements-v3.10.json"))
        manifest = load_json(os.path.join(pkg_dir, "package-payload-manifest-v3.10.json"))
        ...
        for g in entries:
            ...
            if g.get("leaf_visible"):
                ...
                self.require(found, "leaf-visible requirement present in leaf-requirements-v3.10.json", g.get("gate_id"))

        # normative artifact hashes match frozen manifest
        for art in ["gate-traceability-v3.10.json", "leaf-requirements-v3.10.json",
                    "functional-contract-baseline-v3.10.json", "lab-api-contract-v3.10.json",
                    "scope-freeze-v3.10.json", "batch-manifest-v3.10.json",
                    "build-provenance-policy-v3.10.json", "mechanical-finalizer-v3.10.py"]:
    ...
    def phase_identity_coherence(self, pkg_dir):
        auth = load_json(os.path.join(pkg_dir, "package-identity-v3.10.json"))
        batch = load_json(os.path.join(pkg_dir, "batch-manifest-v3.10.json"))
        scope = load_json(os.path.join(pkg_dir, "scope-freeze-v3.10.json"))
    ...
    def phase_functional_continuity(self, pkg_dir):
        api = load_json(os.path.join(pkg_dir, "lab-api-contract-v3.10.json"))
        baseline = load_json(os.path.join(pkg_dir, "functional-contract-baseline-v3.10.json"))
        batch = load_json(os.path.join(pkg_dir, "batch-manifest-v3.10.json"))
        self.require(api["modules"] == baseline["canonical_contract_modules"],
                     "modules deep-equal v3.8 (zero product API regression)")
        self.require(api["version"] == "v3.10", "api version v3.10")
...
def run_selftest(pkg_dir, real_repo):
    """Execute the v3.10 two-group selftest against the REAL finalizer CLI."""
    results = {}
    FINALIZER = os.path.join(pkg_dir, "mechanical-finalizer-v3.10.py")
    GT = os.path.join(pkg_dir, "gate-traceability-v3.10.json")
    LR = os.path.join(pkg_dir, "leaf-requirements-v3.10.json")
    SF = os.path.join(pkg_dir, "scope-freeze-v3.10.json")
    BM = os.path.join(pkg_dir, "batch-manifest-v3.10.json")
    FB = os.path.join(pkg_dir, "functional-contract-baseline-v3.10.json")
    API = os.path.join(pkg_dir, "lab-api-contract-v3.10.json")
    BP = os.path.join(pkg_dir, "build-provenance-policy-v3.10.json")
    IA = os.path.join(pkg_dir, "package-identity-v3.10.json")
    LEASE_A = os.path.join(pkg_dir, "lease-plan-lab-a-v3.10.json")
    LEASE_B = os.path.join(pkg_dir, "lease-plan-lab-b-v3.10.json")
...
""")
    ...
    def finalize(repo, lab, R, lease, evpath, extra_args=None):
        args = ["python3", FINALIZER, "--mode", "reproduce", "--repo", repo,
                ...
                "--batch", "20260808T233306Z-DOGFOOD004-LAB-V3_10",
                ...
                os.path.join(pkg_dir, "acceptance-verifier-v3.10.py")]
        if extra_args:
    ...
    R = commit_all(repo, "r")
    p = finalize(repo, "LAB-A", R, LEASE_A, "receipts/headless-lab-results-v3.10/lab-a-controller-evidence.json")
    out = parse(p)
    ...
    R = commit_all(repo, "r")
    p = finalize(repo, "LAB-B", R, LEASE_B, "receipts/headless-lab-results-v3.10/lab-b-controller-evidence.json")
    out = parse(p)
    ...
    args = ["python3", FINALIZER, "--mode", "reproduce", "--repo", repo,
            ...
            "--controller-dir", os.path.join(base, "ctl"),
            "--evidence-path", "receipts/headless-lab-results-v3.10/lab-b-controller-evidence.json",
            "--verifier-path", os.path.join(pkg_dir, "acceptance-verifier-v3.10.py")]
